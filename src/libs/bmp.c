...
#define H_BYTES_SIZE 54

static BMPv3_STATUS BMP_LAST_ERR_CODE = BMPv3_OK;

static const char* BMP_ERRS[] = {
        "",
        "General error",
...
const char* BMP_get_error_declaration() {
    if (BMP_LAST_ERR_CODE > 0 && BMP_LAST_ERR_CODE < BMPv3_ERR_NUMBER) {
        return BMP_ERRS[BMP_LAST_ERR_CODE];
    } else {
        return NULL;
    }
}

BMPv3* read_BMPv3_f(char* fname) {
    BMPv3* bmp;
    FILE* f;
    long int size_palettes = 0;
    if (fname == NULL) {
        BMP_LAST_ERR_CODE = BMPv3_INVALID_ARG;
        return NULL;
    }
    bmp = (BMPv3*)calloc(1, sizeof(BMPv3));
    if (bmp == NULL) {
        BMP_LAST_ERR_CODE = BMPv3_OUT_OF_MEM;
        return NULL;
    }
    f = fopen(fname, "rb");
    if (f == NULL) {
        BMP_LAST_ERR_CODE = BMPv3_F_NOT_FOUND; free(bmp);
        return NULL;
    }
    if (read_header(bmp, f) != BMPv3_OK || bmp->header.magic != 0x4D42) {
        BMP_LAST_ERR_CODE = BMPv3_INVALID_F;
        fclose(f);
        free(bmp);
        return NULL;
    }
    if (bmp->header.b_p_p == 8) {
        size_palettes = BMP_size_palettes_8;
    }
    if ((bmp->header.b_p_p != 24 && bmp->header.b_p_p != 8)
        || bmp->header.compression_type != 0 || bmp->header.header_size != 40) {
        BMP_LAST_ERR_CODE = BMPv3_F_NOT_SUP;
        fclose(f);
        free(bmp);
        return NULL;
    }
    if (size_palettes > 0) {
        bmp->palette = (unsigned char*)malloc(size_palettes * sizeof(unsigned char));
        if (bmp->palette == NULL) {
            BMP_LAST_ERR_CODE = BMPv3_OUT_OF_MEM;
            fclose(f);
            free(bmp);
        ...
        if (fread(bmp->palette, sizeof(unsigned char), size_palettes, f) != size_palettes) {
            BMP_LAST_ERR_CODE = BMPv3_INVALID_F;
            fclose(f);
            free(bmp->palette);
            free(bmp);
...
int	read_header(BMPv3* bmp, FILE* f) {
    if (bmp == NULL || f == NULL) {
        return BMPv3_INVALID_ARG;
    }
    unsigned char header_bytes[H_BYTES_SIZE];
    if (fread(header_bytes, H_BYTES_SIZE, 1, f) != 1) {
    ...
    bmp->header.b_p_p = get_2byte_int(28, header_bytes);
...
void write_BMPv3_file(BMPv3* bmp, char* fname) {
    FILE* f;
    long int size_palettes = 0;
    if (bmp->header.b_p_p == 8) {
        size_palettes = BMP_size_palettes_8;
    ...
    f = fopen(fname, "wb");
    if (f == NULL) {
        BMP_LAST_ERR_CODE = BMPv3_IO_ERROR;
        return;
    }
    ...
    if (size_palettes > 0) {
        if (fwrite(bmp->palette, sizeof(unsigned char), size_palettes, f) != size_palettes) {
            BMP_LAST_ERR_CODE = BMPv3_IO_ERROR;
            fclose(f);
            return;
        }
...
int write_header(BMPv3* bmp, FILE* f) {
    ...
    unsigned char array_of_bytes[H_BYTES_SIZE];
    ...
    write_2byte_hex(bmp->header.b_p_p, 28, array_of_bytes);
    ...
    if (fwrite(array_of_bytes, H_BYTES_SIZE, 1, f) != 1) {
        return BMPv3_IO_ERROR;
    }
    return BMPv3_OK;
}
